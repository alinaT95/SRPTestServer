...
func (s *SRP) calculateU() (*big.Int, error) {
	...
	trimmedHexPublicA := fmt.Sprintf("%x", s.ephemeralPublicA)
	//trimmedHexPublicA := serverStyleHexFromBigInt(s.ephemeralPublicA)
	trimmedHexPublicB := fmt.Sprintf("%x", s.ephemeralPublicB)
	//trimmedHexPublicB := serverStyleHexFromBigInt(s.ephemeralPublicB)
	arg := trimmedHexPublicA + trimmedHexPublicB
	fmt.Println("ARG =")
	//fmt.Println(arg)

	data, err1 := hex.DecodeString(arg)
	if err1 != nil {
		panic(err1)
	}
	fmt.Println(len(arg))
	...
	_, err := h.Write(data/*fmt.Sprintf("%s%s", trimmedHexPublicA, trimmedHexPublicB)*/)
	if err != nil {
